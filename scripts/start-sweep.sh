#!/usr/bin/env bash
# In-pod launcher for real-weight concurrency sweep (TP16 or PP2).
# Avoids kubectl-exec argv matching kill patterns.
set -euo pipefail

RESULT="${RESULT_DIR:-/tmp/vllm-bench-sweep-512}"
...
echo "SWEEP_PID=$(cat "$RESULT/sweep.pid") RESULT_DIR=$RESULT"
sleep 4
