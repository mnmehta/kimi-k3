#!/usr/bin/env bash
# Deploy multi-node SGLang with TP=8, PP=4 (4×8 H200 = 32 GPUs).
# Same flags as tp8-pp2 but with 4 pipeline stages across 4 nodes.
#
# Usage:
#   export KUBECONFIG=...
#   ./scripts/deploy-sglang-pp4.sh
...
export MAMBA_FULL_MEMORY_RATIO="${MAMBA_FULL_MEMORY_RATIO:-0.90}"
export MAX_RUNNING_REQUESTS="${MAX_RUNNING_REQUESTS:-256}"

# NCCL tuning for cross-node PP P2P
...
echo "==> SGLang PP4 deployment (H200 recipe flags): TP=${TP_SIZE} PP=${PP_SIZE} NNODES=${NNODES}"
...
echo "    CONTEXT_LENGTH=${CONTEXT_LENGTH} MAMBA_SSM_DTYPE=${MAMBA_SSM_DTYPE} MAMBA_FULL_MEMORY_RATIO=${MAMBA_FULL_MEMORY_RATIO} MAX_RUNNING_REQUESTS=${MAX_RUNNING_REQUESTS}"
...
for i in $(seq 0 $((NNODES - 1))); do
  kubectl -n "$NS" cp \
    "$ROOT/scripts/run-sglang-kimi-k3-tp8-pp4.sh" \
...
# Env passed to run-sglang-kimi-k3-tp8-pp4.sh
...
SERVE_ENV+=" CONTEXT_LENGTH=$CONTEXT_LENGTH MAMBA_SSM_DTYPE=$MAMBA_SSM_DTYPE MAMBA_FULL_MEMORY_RATIO=$MAMBA_FULL_MEMORY_RATIO MAX_RUNNING_REQUESTS=$MAX_RUNNING_REQUESTS"
SERVE_ENV+=" NCCL_IB_HCA=$NCCL_IB_HCA NCCL_IB_GID_INDEX=$NCCL_IB_GID_INDEX"
SERVE_ENV+=" NCCL_DMABUF_ENABLE=$NCCL_DMABUF_ENABLE"

echo "==> Starting SGLang on all ranks (rank-3..1 first, then rank-0)"
...
for _ in $(seq 1 360); do
  if kubectl -n "$NS" exec "${STS_NAME}-0" -- \
    ...
    echo
    echo
    echo "==> SGLang PP4 ready. Run benchmarks with:"
