#!/usr/bin/env bash
# In-pod serve script for Kimi-K3 on SGLang (multi-node).
#
# Based on the exact H200 unified/low-latency command from:
...
#
# Official H200 command (verbatim):
...
#
# Override for tp8-pp2: --tp-size 8 --pp-size 2 --ep-size 8
#
# Required env:
...
# Cap context length to match vLLM benchmark (saves KV cache memory).
CONTEXT_LENGTH="${CONTEXT_LENGTH:-8192}"
# Halve Mamba SSM state size → ~2× max_running_requests (26 → ~52+).
MAMBA_SSM_DTYPE="${MAMBA_SSM_DTYPE:-bfloat16}"
# Max concurrent sequences (vLLM PP2 uses --max-num-seqs 256).
...
# Cross-node PP P2P (from vLLM PP2 experience on these nodes)
...
# ---------- Metadata banner ----------
echo "Launching SGLang serve (Kimi-K3):"
...
echo "  NCCL_DMABUF_ENABLE=$NCCL_DMABUF_ENABLE NCCL_IB_HCA=${NCCL_IB_HCA:-<unset>} NCCL_IB_GID_INDEX=${NCCL_IB_GID_INDEX:-<unset>}"
