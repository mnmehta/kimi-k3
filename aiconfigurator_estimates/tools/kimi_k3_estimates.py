...
best.json, commands.txt (equivalent CLI calls), and run.log. Missing profiles
...
def save_report(output, rows, ground_truth, repo):
    """Write a Markdown AIC-versus-measured report for the four 16-GPU runs."""
    report_rows = []
    for row in rows:
    ...
    def pct(aic, gt):
        if aic is None or gt == 0:
            return "—"
        return f"{100 * (aic - gt) / gt:+.1f}%"
    ...
    lines = [
        ...
        "",
        f"Ground-truth repository: `{Path(repo).resolve()}`",
    ...
    for item in report_rows:
        if item["status"] != "ok":
            lines.append(f"| {item['strategy']} | {item['target_c']} | — | — | estimate failed | — | — | — | — | — | — |")
            continue
        lines.append(
            f"| {item['strategy']} | {item['target_c']} | {item['aic_c']} | "
            f"{'yes' if item['target_c'] == item['aic_c'] else 'NO'} | "
            f"{item['gt_ttft']:.1f} / {item['aic_ttft']:.1f} | {pct(item['aic_ttft'], item['gt_ttft'])} | "
            f"{item['gt_tpot']:.1f} / {item['aic_tpot']:.1f} | {pct(item['aic_tpot'], item['gt_tpot'])} | "
            f"{item['gt_itl']:.1f} | {item['gt_tok_s']:.1f} / {item['aic_tok_s']:.1f} | "
            f"{pct(item['aic_tok_s'], item['gt_tok_s'])} |"
        )
    lines.extend(
        [
            "",
    ...
    for strategy in REPORT_DIRS:
        items = [item for item in report_rows if item["strategy"] == strategy and item["status"] == "ok"]
        if not items:
            lines.append(f"| {strategy} | — | — | — | 0 |")
            continue
        mean_abs = lambda key, truth: sum(abs((item[key] - item[truth]) / item[truth]) for item in items) / len(items)
        lines.append(
            f"| {strategy} | {100 * mean_abs('aic_ttft', 'gt_ttft'):.1f}% | "
            f"{100 * mean_abs('aic_tpot', 'gt_tpot'):.1f}% | "
            f"{100 * mean_abs('aic_tok_s', 'gt_tok_s'):.1f}% | {len(items)} |"
        )
    lines.extend(
        [
            "",
