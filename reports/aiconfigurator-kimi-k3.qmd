...
records = json.loads(Path("aiconfigurator-comparison.json").read_text())
...
The complete machine-readable comparison is in [`aiconfigurator-comparison.json`](aiconfigurator-comparison.json); raw AIC rows and per-operation source tags are retained in the AIConfigurator result artifact.
